...
def test_create_individual_basic():
...
def test_create_individual_single_item():
    items = [Item(5, 3)]
    individual = create_individual(items)
...
def test_create_individual_multiple_calls_different_results():
...
def test_create_population_empty_items():
    items = []
    with pytest.raises(ValueError, match="Input list must have values"):
        create_population(items)

def test_create_population_none_items():
    with pytest.raises(ValueError, match="Input list must have values"):
        create_population(None)

def test_create_population_diversity():
...
def test_create_population_large_items():
...
def test_create_population_with_different_item_types():
...
def test_individual_randomness_distribution():
    items = [Item(i, i+1) for i in range(100)]
...
def test_create_population_returns_new_instances():
