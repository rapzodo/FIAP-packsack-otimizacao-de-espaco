...
class TestFitness:
    ...
    def test_valid_solution_exact_capacity(self):
        individual = [1, 0, 1]
        items = [Item(30, 5), Item(0, 8), Item(25, 5)]
        capacity = 10
        assert evaluate_fitness(individual, items, capacity) == 55

    def test_partial_selection_exceeds_capacity(self):
        individual = [1, 1, 0]
        items = [Item(10, 4), Item(40, 8), Item(5, 2)]
        capacity = 10
        assert evaluate_fitness(individual, items, capacity) == INVALID_SOLUTION_SCORE

    def test_items_with_zero_value(self):
        individual = [1, 1]
    ...
    def test_all_items_selected_within_capacity(self):
        individual = [1, 1, 1]
    ...
    def test_negative_values_or_weights(self):
        individual = [1, 1]
    ...
    def test_item_class_functionality(self):
        item = Item(15, 8)
        assert item.value == 15
        assert item.weight == 8
        assert str(item) == "Item(value=15, weight=8)"

    def test_fitness_with_mixed_selection(self):
